...
void curar_equipo(equipo_t equipo) {
    bool no_lesionado = false;

    // CASO: Defensor no/si lesionado
    if (equipo->defensor->lesionado == no_lesionado) {
        equipo->defensor->energia = ENERGIA_OPTIMA;
    } else { //En el caso que este lesionado:
        free(equipo->defensor);
        
        struct _jugador *pepe=malloc(sizeof(struct _jugador));
        pepe->energia = ENERGIA_OPTIMA;
        pepe->lesionado = false;

        equipo->defensor = pepe;
    }

    // CASO: Delantero no/si lesionado
    if (equipo->delantero->lesionado == no_lesionado) {
        equipo->delantero->energia = ENERGIA_OPTIMA;
    } else {
        free(equipo->delantero);

        struct _jugador *juan=malloc(sizeof(struct _jugador));
        juan->energia = ENERGIA_OPTIMA;
        juan->lesionado = false;
    
        equipo->delantero = juan;
    }
}